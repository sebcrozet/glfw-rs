...
pub extern "C" fn error_callback(error: c_int, description: *c_char) {
    do local_data::get(error_fun_tls_key) |data| {
        do data.as_ref().map |&ref cb| {
            unsafe { (**cb)(cast::transmute(error as c_int), str::raw::from_c_str(description)) }
        };
    }
}
...
pub extern "C" fn monitor_callback(monitor: *ffi::GLFWmonitor, event: c_int) {
    do local_data::get(monitor_fun_tls_key) |data| {
        do data.as_ref().map |&ref cb| {
            unsafe { (**cb)(&Monitor { ptr: monitor }, cast::transmute(event as c_int)) }
        };
    }
}
...
window_callback!(fn mouse_button_callback(button: c_int, action: c_int, mods: c_int)        => mouse_button_fun(cast::transmute(button as i8), cast::transmute(action as i8), Modifiers { values: mods }))
...
window_callback!(fn key_callback(key: c_int, scancode: c_int, action: c_int, mods: c_int)   => key_fun(cast::transmute(key as i16), scancode, cast::transmute(action as i8), Modifiers { values: mods }))
